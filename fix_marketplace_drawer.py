...
def fix_marketplace_drawer():
    filepath = 'marketplace.html'
    with codecs.open(filepath, 'r', encoding='utf-8') as f:
        html = f.read()

    # We need to completely rewrite the drawer, time calculations, and state management in marketplace.html
    # Let's extract the <script> tags at the bottom and replace them.

    # Find the main script block
    new_script = """
window.viewListModal = {
    isOpen: false,
    init: function() {
    ...
    open: function() {
        ...
        document.getElementById('list-drawer').classList.remove('opacity-0', 'scale-95', 'pointer-events-none');
        document.getElementById('list-drawer').classList.add('opacity-100', 'scale-100', 'pointer-events-auto');
        document.getElementById('drawer-overlay').classList.remove('opacity-0', 'pointer-events-none');
        document.getElementById('drawer-overlay').classList.add('opacity-100', 'pointer-events-auto');
        document.body.style.overflow = 'hidden';
    },
    close: function() {
        this.isOpen = false;
        document.getElementById('list-drawer').classList.remove('opacity-100', 'scale-100', 'pointer-events-auto');
        document.getElementById('list-drawer').classList.add('opacity-0', 'scale-95', 'pointer-events-none');
        document.getElementById('drawer-overlay').classList.remove('opacity-100', 'pointer-events-auto');
...
window.toggleListDrawer = function(e) {
    if(e) e.stopPropagation();
    window.viewListModal.toggle();
};
...
window.renderListDrawer = function() {
    ...
    let html = `
        <div class="px-4 py-3 border-b border-outline-variant flex justify-between items-center bg-surface-container-lowest rounded-t-xl shrink-0">
            ...
            <button onclick="window.viewListModal.close()" class="text-secondary hover:text-primary p-1"><span class="material-symbols-outlined text-[18px]">close</span></button>
        </div>
        <div class="p-4 overflow-y-auto flex-1 space-y-3 overscroll-contain">
    `;
    
    if (window.draftLayoverList.length === 0) {
        html += `<p class="text-xs text-secondary text-center py-4">No experiences added yet.</p>`;
    } else {
    ...
    html += `</div>
        <div class="p-3 bg-surface-container-lowest border-t border-outline-variant flex justify-between gap-3 shrink-0 rounded-b-xl">
            <button onclick="window.viewListModal.close()" class="flex-1 py-2 rounded-lg border border-outline-variant font-bold text-xs text-secondary hover:bg-surface-container-high transition-colors">Cancel</button>
...
window.updateTimeCalculations = function() {
    ...
    // Parse layover duration robustly
    ...
    if (layoverDurationRaw.includes('h') || layoverDurationRaw.includes('H')) {
        ...
        totalLayoverMins = parseFloat(layoverDurationRaw) * 60; // assume hours if just number
    }
    
    // Parse departure time robustly
...
// Initialize modal DOM structures immediately
document.addEventListener('DOMContentLoaded', () => {
    window.viewListModal.init();
});
"""
