...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.renderItinerary = function() {
        ...
        if (layoverList.length === 0) {
            ...
            layoverList.forEach((item, index) => {
                let statusColor = "border-outline-variant/30";
                let statusBadge = "";
                ...
                if (item.startTime && item.endTime) {
                    if (item.startTime >= item.endTime) {
                        ...
                        statusBadge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-error-container text-error ml-2 flex items-center gap-1"><span class="material-symbols-outlined text-[12px]">error</span> Invalid Time</span>`;
                    }
                }
                if (!hasError && index > 0 && item.startTime && layoverList[index-1].endTime) {
                    if (item.startTime < layoverList[index-1].endTime) {
                        hasError = true;
                        statusColor = "border-error";
                        statusBadge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-error-container text-error ml-2 flex items-center gap-1"><span class="material-symbols-outlined text-[12px]">warning</span> Overlap</span>`;
                    }
                }

                html += `
                    <div class="bg-surface-container-lowest border ${statusColor} rounded-xl p-md flex flex-col gap-3 group hover:border-primary/40 transition-colors shadow-sm relative">
                        ...
                        <div class="flex gap-4 items-center bg-surface p-3 rounded-lg border border-outline-variant/50 sm:ml-12">
                            <div class="flex flex-col w-full sm:w-32">
                                <label class="text-[10px] font-bold text-secondary uppercase tracking-wider mb-1">Start Time</label>
                                <input type="time" value="${item.startTime || ''}" onchange="updateStart(${index}, this.value)" class="bg-white border border-outline-variant rounded px-2 py-1 text-sm text-primary font-medium focus:border-primary outline-none">
                            </div>
                            <div class="flex flex-col w-full sm:w-32">
                                <label class="text-[10px] font-bold text-secondary uppercase tracking-wider mb-1">End Time</label>
                                <input type="time" value="${item.endTime || ''}" onchange="updateEnd(${index}, this.value)" class="bg-white border border-outline-variant rounded px-2 py-1 text-sm text-primary font-medium focus:border-primary outline-none">
                            </div>
                        </div>
    ...
    window.updateStart = function(i, val) {
        layoverList[i].startTime = val;
        window.saveAndRender();
    };
    window.updateEnd = function(i, val) {
        layoverList[i].endTime = val;
        window.saveAndRender();
    };
