...
def fix_spa_durations():
    ...
    # If there is a filter-duration in spa.html (just in case)
    html = re.sub(
    ...
    script_inject = """
        ...
        document.addEventListener('DOMContentLoaded', populateSpaDurations);
"""
    ...
    html = html.replace('document.addEventListener(\'DOMContentLoaded\', renderItems);', 
                       'populateSpaDurations();\n        document.addEventListener(\'DOMContentLoaded\', renderItems);\n        renderItems();')

    with codecs.open(filepath, 'w', encoding='utf-8') as f:
        f.write(html)
    print("Fixed spa.html duration dropdowns.")
